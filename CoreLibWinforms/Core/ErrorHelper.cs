...
namespace CoreLibWinforms.Core
{
    ...
    public class _sample()
    {
        ...
        {
            // 基本的なエラー表示（ユーザー向けのみ）
            ErrorHelper.ShowError("ファイルを開けませんでした。");
            ...
            {
                var errorInfo = new ErrorInfo("計算中にエラーが発生しました。", ex)
                    .WithErrorCode("CALC001")
                    .WithSource("Calculator")
                    .WithSeverity(ErrorSeverity.Error)
                    .WithData("Operation", "Division")
                    .WithData("Input", "10 / 0");

                // ユーザーと開発者の両方向けダイアログを表示
                ErrorHelper.HandleError(errorInfo, true, true);
            }

            // ヘルパーメソッドを使ったエラーハンドリング
            ErrorHelper.TryCatch(
                () => {
                    // 処理
                    File.ReadAllText("存在しないファイル.txt");
                },
                "ファイル操作中にエラーが発生しました。",
                true,   // ユーザーダイアログを表示
                true    // 開発者ダイアログを表示
            ...
            //// 非同期処理でのエラーハンドリング
            //await ErrorHelper.TryCatchAsync(
            //    async () => {
            //        await Task.Delay(100);
            //        throw new InvalidOperationException("非同期処理エラー");
            //    },
            //    "処理中にエラーが発生しました。",
            //    true,   // ユーザーダイアログを表示
            //    true    // 開発者ダイアログを表示
